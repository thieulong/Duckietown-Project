...
class FollowClosestSignNode:
    def __init__(self):
        rospy.init_node('follow_closest_sign')
    ...
    def tag_callback(self, tag_array):
        if len(tag_array.detections) > 0:
            distances = [tag.transform.translation.y for tag in tag_array.detections]
            min_distance_index = distances.index(min(distances))
            tag = tag_array.detections[min_distance_index]
...
if __name__ == '__main__':
    try:
        align_node = FollowClosestSignNode()
        rospy.spin()
