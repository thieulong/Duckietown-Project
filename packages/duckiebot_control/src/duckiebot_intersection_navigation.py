...
class IntersectionNavigation:
    ...
    def handle_movement(self, direction):
        if direction == "forward":
            ...
            self.move_duckiebot_forward(duration=1.2)
            self.turn_duckiebot_right(duration=0.8)
        elif direction == "left":
            self.move_duckiebot_forward(duration=1.5)
            self.turn_duckiebot_left(duration=1.2)
