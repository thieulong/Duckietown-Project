...
class FollowTargetSignNode:
    def __init__(self):
        rospy.init_node('target_sign_following')
    ...
    def tag_callback(self, tag_array):
        if len(tag_array.detections) > 0:
            tag_ids = [tag.tag_id for tag in tag_array.detections]
            target_id = 25
            if target_id in tag_ids: 
                id_index = tag_ids.index(25)
                tag = tag_array.detections[id_index]
...
if __name__ == '__main__':
    try:
        align_node = FollowTargetSignNode()
        rospy.spin()
