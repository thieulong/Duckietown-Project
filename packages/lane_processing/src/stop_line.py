...
class StopLineFilterNode(DTROS):
    def __init__(self, node_name):
        ...
        self.sub_segs = rospy.Subscriber("/duckiebot/line_detector_node/segment_list", SegmentList, self.cb_segments)
        self.sub_lane = rospy.Subscriber("/duckiebot/lane_filter_node/lane_pose", LanePose, self.cb_lane_pose)
        self.sub_mode = rospy.Subscriber("fsm_node/mode", FSMState, self.cb_state_change)
        # self.pub_stop_line_reading = rospy.Publisher("/duckiebot/stop_line_recognition_node/stop_line_reading", StopLineReading, queue_size=1)
        # self.pub_at_stop_line = rospy.Publisher("/duckiebot/stop_line_recognition_node/at_stop_line", BoolStamped, queue_size=1)
        self.pub_stop_line_reading = rospy.Publisher("/duckiebot/stop_line_filter_node/stop_line_reading", StopLineReading, queue_size=1)
        self.pub_at_stop_line = rospy.Publisher("/duckiebot/stop_line_filter_node/at_stop_line", BoolStamped, queue_size=1)
    ...
    def cb_segments(self, segment_list_msg):
        ...
        for segment in segment_list_msg.segments:
            ...
            stop_line_y_accumulator += avg_y  # output covariance and not just mean
            good_seg_count += 1.0
...
if __name__ == "__main__":
    lane_filter_node = StopLineFilterNode(node_name="stop_line_recognition_node")
    rospy.spin()