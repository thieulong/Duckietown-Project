...
class CustomLaneDetection:
    def __init__(self):
        ...
        self.pub_edges_red = rospy.Publisher('/image_lines_red', Image, queue_size=10)  # New publisher for red lines
    ...
    def lane_processing_callback(self, msg):
        ...
        red_filtered = cv2.bitwise_and(image_cropped, image_cropped, mask=mask_red)
    ...
    def output_lines_red(self, image, lines):
        output = np.copy(image)
        if lines is not None:
            for i in range(len(lines)):
                l = lines[i][0]
                ...
                cv2.circle(output, (l[0], l[1]), 2, (0, 255, 0))
                cv2.circle(output, (l[2], l[3]), 2, (255, 0, 0))
        return output
